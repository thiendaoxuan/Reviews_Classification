...
from dataHelper_sentiment import load_data,ENTITY, tmp_sequence_length, VOCAB, VOCAB_INV, BOTH
...
for aspect in BOTH:
    ...
    entity = aspect.split('#')[0]
    attribute = aspect.split('#')[1]

    entityFolder = 'TrainedModel/sentiment/' + entity
    saved_model = []
    if os.path.exists(entityFolder) :
        saved_model = [f for f in os.listdir(entityFolder)]

    # DO not retrain

    if attribute + '.h5' in saved_model:
        continue

    x, y, x_test, y_test = load_data(aspect)
    c = list(zip(x, y))
    ...
    entity = aspect.split('#')[0]
    attribute = aspect.split('#')[1]
    directory = 'TrainedModel/sentiment/' +entity
    if not os.path.exists(directory):
        os.makedirs(directory)


    model.save(directory + '/'+ attribute + '.h5')


    print ("-------------------------")
    print (aspect)
    print "Training information. " + "Postitive : " + str(y.tolist().count(1)) + " . Negative : " + str(y.tolist().count(0))
    print "Testing information. " + "Postitive : " + str(y_test.tolist().count(1)) + " . Negative : " + str(y_test.tolist().count(0))
